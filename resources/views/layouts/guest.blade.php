    ...
    <body class="font-sans text-gray-900 antialiased bg-slate-100">
        <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 bg-slate-100">
            <div class="flex flex-col items-center text-center">
                <a href="/">
                    <img src="{{ asset('images/couja-logo.png') }}" alt="CoUJA Logo" class="w-24 h-24 object-contain shadow-md rounded-full bg-white p-1">
                </a>
                <h1 class="text-lg font-black text-slate-900 mt-3">
                    {{ __('Comilla University Journalist Association') }}
                </h1>
                <p class="text-xs font-bold text-red-600 mt-0.5">
                    {{ __('Unwavering on the Path of Truth & Justice') }} • {{ __('Estd. 2013') }}
                </p>
            </div>

            <div class="w-full sm:max-w-md mt-6 px-6 py-6 bg-white shadow-lg overflow-hidden sm:rounded-2xl border border-slate-200">
                {{ $slot }}
            </div>
        </div>
    </body>
